...
// const credsCache = new NodeCache({ stdTTL: 43200, checkperiod: 1200 });
...
class OneDrive {
  ...
  items = async (pool) => {
    try {
      ...
      return {
        listChildren: async (args) => api.listChildren({ accessToken, ...args }),
        createFolder: async (args) => api.createFolder({ accessToken, ...args }),
...
simpleFunc = async () => {
  ...
  const res = await MO.items('myPool').car();
  // console.log(res);
}
